...
public class OrdenacaoQuickSort<T extends Comparable<T>> extends OrdenacaoAbstract<T> {
	...
	public int retornaPivoExecucao() {
		return quickSortPivo(0, getInfo().length-1);
	}
	
	private int quickSortPivo(int inicio, int fim) {
		if (inicio < fim) {
	...
	private int particionar(int inicio, int fim) {
		...
		while (true) {
			do {
				// incremente o indice de A enquanto o info[a] for menor que pivo.
				a++;

...
public class OrdenacaoMergeSort<T extends Comparable<T>> extends OrdenacaoAbstract<T> {
	...
	private void merge(int inicio, int fim, int meio) {
		...
		if (inicio == 0 && fim == getInfo().length - 1) {
			String valoresEsquerda = "Valores que sobraram na esquerda: [";
			for (int iEsc = cEsq; iEsc < esquerda.length; iEsc++) {
				valoresEsquerda += esquerda[iEsc];
				valoresEsquerda += ", ";
			}
			valoresEsquerda += "]";
			System.out.println(valoresEsquerda);
			
			String valoresDireita = "Valores que sobraram na direita: [";
			for (int iDir = cDir; iDir < direita.length; iDir++) {
				valoresDireita += direita[iDir];
				valoresDireita += ", ";
			}
			valoresDireita += "]";
			System.out.println(valoresDireita);
		}

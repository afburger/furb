...
//	}
	...
	public void inverter() {
		...
		T[] invertida = (T[]) new Object[info.length];
		int indiceInverido = 0;
		for (int indice = tamanho -1; indice > -1; indice--) {
			invertida[indiceInverido] = info[indice];
			indiceInverido++;
		}
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + Arrays.deepHashCode(info);
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ListaEstaticaGenerica<T> listaComparacao = (ListaEstaticaGenerica<T>) obj;
		if (this.getTamanho() != listaComparacao.getTamanho())
			return false;

		for (int indice = 0; indice < this.getTamanho(); indice++) {
			if (!(this.info[indice].equals(listaComparacao.info[indice]))) {
				return false;
			}
		}
